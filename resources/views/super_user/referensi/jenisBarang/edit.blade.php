...
@section('content')
<div class="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-100 to-blue-200 p-6">
    <div class="w-full max-w-lg bg-white p-8 rounded-2xl shadow-2xl transform transition-all hover:scale-105">
        ...
        <form action="{{ route('superuser.jenisBarang.update', $jenisBarang->jns_brg_kode) }}" method="POST">
            ...
            <button type="submit" class="w-full bg-gradient-to-r from-indigo-500 to-blue-500 text-white py-3 rounded-lg shadow-md text-lg font-semibold
                                    transition-all duration-300 hover:from-blue-500 hover:to-indigo-500 hover:shadow-xl">Update</button>
        </form>
    </div>
</div>
@endsection
