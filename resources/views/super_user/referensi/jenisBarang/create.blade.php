...
@section('content')
<div class="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-100 to-indigo-200 p-6">
    <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-500 hover:scale-105">
        <h2 class="text-4xl font-extrabold text-indigo-700 text-center mb-6">Tambah Jenis Barang</h2>
        <form action="{{ route('superuser.jenisBarang.store') }}" method="POST" class="space-y-6">
            ...
            <button type="submit"
                class="w-full py-3 bg-gradient-to-r from-indigo-500 to-blue-500 text-white text-lg font-semibold rounded-xl shadow-md hover:opacity-90 transform transition-all duration-300">
                Simpan
            </button>
        </form>
    </div>
